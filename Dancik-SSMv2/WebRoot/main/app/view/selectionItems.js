...
//				container.find("#ssm_orderModeIcon").closest(".commandButton").remove();
//				container.find("#ssm_deleteModeIcon").closest(".commandButton").remove();
//				container.find("#ssm_addModeIcon").closest(".commandButton").remove();
...
//						Show Discontinued and Previously Ordered if applicable
						...
						if (item.prev_ordered_flag == "Y"){
							newRow.find(".previouslyOrdered").show();
							if(!_this.config.allow_ordering_previously_ordered_items){
								newRow.find('.orderCheck').remove();
							}
						}
...
//				Pass potentially existing search term and boolean to the item search view via a paylaod
			...
			// --------------------------------------------------------
			// -- Toggle pricing info
			container.on('click', '#ssm_chkbx_ShowPrices', function(){
				//store Show Prices checked state 
				$App.Model('Selection').showPrices = $(this).attr("checked");
				if ($(this).is(":checked")){
					$(".ssmPricingInfo").show();
...
//			Initiate Order Mode
			container.find("#ssm_orderModeIcon").click(function(){
				// hide price columns
				if(container.find("#ssm_chkbx_ShowPrices").is(":checked")){
					container.find("#ssm_chkbx_ShowPrices").prop('checked',false);
					container.find(".ssmPricingInfo").hide();
					container.find(".selectionItemList").hide();
					container.find(".colWidth-1").removeClass("colWidth-100");
				}
...
//			Add functionality to Order Checkboxes
			...
			// persist the show prices checkbox state
			if($App.Model('Selection').showPrices == "checked"){ $("#ssm_chkbx_ShowPrices")[0].click();	}
		},
...
//			 if the override list is changed, wipe out the override price
			container.find("#ssm_price_codes_select").off();
			container.find("#ssm_price_codes_select").on('change', function () {
				container.find('input[name=ssm_PriceOverride]').val('');
			});
//			if the override price is changed, wipe out the override list
			container.find('input[name=ssm_PriceOverride]').off();
			container.find('input[name=ssm_PriceOverride]').on('blur', function () {
				if($(this).val() != ""){
					container.find('#ssm_price_codes_select option').each(function(){
						if ($(this).val()==""){
